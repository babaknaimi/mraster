---
title: "Handling big spatial (raster/image) data in R (mraster)"
author: "Babak Naimi"
date: "31/03/2018"
output: 
  md_document:
...
Let's try a raster file that can be loaded into memory
Different solutions are tested:
* solution1: when data are loaded in memory
* solution2: when data are in disk but will be handled by the method implemented in the raster package
* solution3: This is the solution offered by this package, i.e., data are in disk and will be handled by mapping the memory to the file (mraster package)
...
c1 <- sum(b$time[b$expr == "in.disk.using.raster[indx]"]) / sum(b$time[b$expr == "getRasterValues(map.to.file.using.mraster, indx)"]) 
c2 <- sum(b$time[b$expr == "in.memory.using.raster[indx]"]) / sum(b$time[b$expr == "getRasterValues(map.to.file.using.mraster, indx)"]) 

cat("The solution offered in this proposal is",c1,
    "times faster than when the raster object \n is on the disk, accessed from the raster package")

cat("The solution offered in this proposal is",c2,
    "times faster than when the raster object \n is loaded in the memory,", 
    "accessed from the raster package")
...
cat("The solution offered in this proposal is",c2,
    "times faster than when the raster object is \n loaded in the memory,", 
    "accessed from the raster package")
...
In this example, we try a raster file that cannot be loaded in the memory
Two solutions are tested:
* solution1: when data are in disk and will be handled by the method implemented in the raster package
* solution2: This is the solution offered by this package, i.e., data are in disk and will be handled by mapping the memory to the file (mraster package)
